...
void cache_sim_t::init()
{
  ...
  freq = new size_t[sets*ways]();	//edit
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++){
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
    	freq[idx*ways + i]++;	//edit
    	size_t min = freq[idx*ways + next_way[idx]];	//adjust next_way
    	for(size_t j = 0; j < ways; j++){
    		if(min > freq[idx*ways + j]){
    			next_way[idx] = j;
    			min = freq[idx*ways + j];
    		}
    	}	//edit
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  freq[idx*ways + next_way[idx]] = 1;	//edit
  size_t min = freq[idx*ways + next_way[idx]];	//adjust next_way
	for(size_t i = 0; i < ways; i++){
		if(min > freq[idx*ways + i]){
			next_way[idx] = i;
			min = freq[idx*ways + i];
		}
	}	//edit
